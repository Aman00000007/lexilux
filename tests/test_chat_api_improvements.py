...
class TestChatVsComplete:
    ...
    def test_complete_raises_if_still_truncated(self, mock_post):
        ...
        with pytest.raises(ChatIncompleteResponseError):
            chat.complete("Write a very long story", max_tokens=5, max_continues=2, ensure_complete=True)
    ...
    def test_continue_delay(self, mock_post):
        ...
        def track_time(*args, **kwargs):
            ...
            if len(call_times) == 1:
                ...
                return Mock(
                    json=lambda: {
                        "choices": [{"message": {"content": "Partial1"}, "finish_reason": "length"}],
                        "usage": {"prompt_tokens": 10, "completion_tokens": 5, "total_tokens": 15},
                    },
                ...
                return Mock(
                    json=lambda: {
                        "choices": [{"message": {"content": "Partial2"}, "finish_reason": "length"}],
                        "usage": {"prompt_tokens": 15, "completion_tokens": 10, "total_tokens": 25},
                    },
                    raise_for_status=lambda: None,
                )
        ...
        if len(call_times) >= 3:
            ...
            assert delay_between_continues >= 0.05, f"Expected delay >= 0.05s, got {delay_between_continues}"
